...
static int msm_hsphy_probe(struct platform_device *pdev)
{
	...
	dev_dbg(dev, "param_override_seq_cnt:qcom,param-override-seq-factory\n");
#else
	phy->param_override_seq_cnt = of_property_count_elems_of_size(
					dev->of_node,
					"qcom,param-override-seq_normal",
					sizeof(*phy->param_override_seq));
	dev_dbg(dev, "param_override_seq_cnt:qcom,param-override-seq_normal\n");
#endif
	...
	if (phy->param_override_seq_cnt > 0) {
		...
		ret = of_property_read_u32_array(dev->of_node,
				...
				phy->param_override_seq_cnt);
		dev_dbg(dev, "qcom,param-override-seq-factory %d\n",
				ret);
#else
		ret = of_property_read_u32_array(dev->of_node,
				"qcom,param-override-seq_normal",
				phy->param_override_seq,
				phy->param_override_seq_cnt);
		dev_dbg(dev, "qcom,param-override-seq_normal %d\n",
				ret);
#endif
                dev_err(dev, "qcom,param-override-seq %02x %02x %02x %02x %02x %02x\n",
	        phy->param_override_seq[0],phy->param_override_seq[1],phy->param_override_seq[2],
                        phy->param_override_seq[3],phy->param_override_seq[4],phy->param_override_seq[5]);
