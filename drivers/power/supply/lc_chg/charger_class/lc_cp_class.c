...
int chargerpump_set_otg(struct chargerpump_dev *chargerpump, bool enable)
{
	if (!chargerpump || !chargerpump->ops)
		return -EINVAL;
	if (chargerpump->ops->set_otg == NULL)
		return -EOPNOTSUPP;
	return chargerpump->ops->set_otg(chargerpump, enable);
}
EXPORT_SYMBOL(chargerpump_set_otg);

int chargerpump_set_ovp_gate(struct chargerpump_dev *chargerpump, bool enable)
{
	if (!chargerpump || !chargerpump->ops)
		return -EINVAL;
	if (chargerpump->ops->set_ovp_gate == NULL)
		return -EOPNOTSUPP;
	return chargerpump->ops->set_ovp_gate(chargerpump, enable);
}
EXPORT_SYMBOL(chargerpump_set_ovp_gate);
...
int chargerpump_get_cp_vendor(struct chargerpump_dev *chargerpump)
{
	if (!chargerpump || !chargerpump->ops)
		return -EINVAL;
	if (chargerpump->ops->get_cp_vendor == NULL)
		return -EOPNOTSUPP;
	return chargerpump->ops->get_cp_vendor(chargerpump);
}
EXPORT_SYMBOL_GPL(chargerpump_get_cp_vendor);
...
MODULE_DESCRIPTION("LC Charger Pump Class Core");
