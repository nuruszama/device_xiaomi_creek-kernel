...
#define QPNP_PON_SET_TYPE(pon)			((pon)->base + 0x11)
#define QPNP_PON_POLARITY_HIGH(pon)		((pon)->base + 0x12)
#define QPNP_PON_POLARITY_LOW(pon)		((pon)->base + 0x13)
#define QPNP_PON_EN_SET(pon)			((pon)->base + 0x15)
...
static irqreturn_t qpnp_kpdpwr_bark_irq(int irq, void *_pon)
{
	...
	struct qpnp_pon *pon_s = (struct qpnp_pon *)_pon;
	rc = qpnp_pon_read(pon_s, QPNP_PON_RT_STS(pon_s), &pon_rt_sts);
	if (rc){
		return rc;
        }
	//------ Detected that the power button has been long pressed
	//------ Possible reboot or combination key warm reset
...
qpnp_pon_request_irqs(struct qpnp_pon *pon, struct qpnp_pon_config *cfg)
{
	...
	case PON_KPDPWR:
		...
		if (cfg->use_bark) {
			// enable kpadpwr irq config
                        rc = qpnp_pon_read(pon, QPNP_PON_EN_SET(pon), &pon_val);
                        if (rc < 0)
                            return rc;

                        pon_val_w = pon_val | 0x4;

                        rc = qpnp_pon_write(pon, QPNP_PON_EN_SET(pon), pon_val_w);
                        if (rc)
                            return rc;

                        rc = qpnp_pon_read(pon, QPNP_PON_SET_TYPE(pon), &pon_val);
                        if (rc < 0)
                            return rc;

                        pon_val_w = pon_val | 0x4;

                        rc = qpnp_pon_write(pon, QPNP_PON_SET_TYPE(pon), pon_val_w);
                        if (rc)
                            return rc;

                        rc = qpnp_pon_read(pon, QPNP_PON_POLARITY_HIGH(pon), &pon_val);
                        if (rc < 0)
                            return rc;

                        pon_val_w = pon_val | 0x4;

                        rc = qpnp_pon_write(pon, QPNP_PON_POLARITY_HIGH(pon), pon_val_w);
                        if (rc)
                            return rc;
