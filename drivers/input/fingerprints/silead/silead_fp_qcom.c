...
static int silfp_parse_dts(struct silfp_data* fp_dev)
{
    int ret;
//#ifndef QSEE_V4
    LOG_MSG_DEBUG(ERR_LOG, "%s xiaomifp_ldo_outputhigh 2\n", __func__);
...
#ifdef BSP_SIL_POWER_SUPPLY_PINCTRL
    LOG_MSG_DEBUG(ERR_LOG, "%s xiaomifp_ldo_outputhigh 2\n", __func__);
    fp_dev->pin.pins_avdd_h = pinctrl_lookup_state(fp_dev->pin.pinctrl, "xiaomifp_ldo_outputhigh");
...
static int silfp_resource_init(struct silfp_data *fp_dev, struct fp_dev_init_t *dev_info)
{
    ...
    if (fp_dev->rst_port >= 0 ) {
        ...
        if (ret < 0) {
            ...
            msleep(10);
            gpio_direction_output(fp_dev->rst_port, 1);
            LOG_MSG_DEBUG(INFO_LOG,"[%s] Enable_reset_gpio.\n",__func__);
        }
    }
