...
struct geni_i2c_dev {
	...
	bool se0_enable_recovery;
	bool se1_enable_recovery;
};
...
#define GPIO0_SDA_NUM (GPIO0_BASE_NUM + 0) // 398 + 0
#define GPIO1_SCL_NUM (GPIO0_BASE_NUM + 1) // 398 + 1
#define GPIO4_SDA_NUM (GPIO0_BASE_NUM + 4) // 398 + 4
#define GPIO5_SCL_NUM (GPIO0_BASE_NUM + 5) // 398 + 5
static void force_i2c0_scl(struct geni_i2c_dev *gi2c)
{
	struct pinctrl *p;
	int i, ret;
	...
	ret = gpio_request(GPIO1_SCL_NUM, "i2c_scl");
	ret |= gpio_request(GPIO0_SDA_NUM, "i2c_sda");
	if (ret < 0) {
		pr_err("%s failed to request i2c scl/sda gpio\n", __func__);
	}

	gpio_direction_input(GPIO0_SDA_NUM);
	for (i = 0; i < 10; i++) {
		gpio_direction_output(GPIO1_SCL_NUM, 1);
		udelay(5);
		if (gpio_get_value(GPIO0_SDA_NUM) == 1) {
			pr_err("%s driver sda successful, cnt = %d\n", __func__, i);
			goto out;
		}
		gpio_direction_output(GPIO1_SCL_NUM, 0);
		udelay(5);
	}
	pr_err("%s failed to recovery i2c\n", __func__);
out:
	gpio_direction_output(GPIO0_SDA_NUM, 1);
	gpio_direction_output(GPIO1_SCL_NUM, 1);
	udelay(1);
	gpio_free(GPIO0_SDA_NUM);
	gpio_free(GPIO1_SCL_NUM);
...
static void force_i2c1_scl(struct geni_i2c_dev *gi2c)
{
        struct pinctrl *p;
        int i, ret;
        p = devm_pinctrl_get_select(gi2c->dev, "sleep");
        ...
        ret = gpio_request(GPIO5_SCL_NUM, "i2c_scl");
        ret |= gpio_request(GPIO4_SDA_NUM, "i2c_sda");
        if (ret < 0) {
                pr_err("%s failed to request i2c scl/sda gpio\n", __func__);
        }

        gpio_direction_input(GPIO4_SDA_NUM);
        for (i = 0; i < 10; i++) {
                gpio_direction_output(GPIO5_SCL_NUM, 1);
                udelay(5);
                if (gpio_get_value(GPIO4_SDA_NUM) == 1) {
                        pr_err("%s driver sda successful, cnt = %d\n", __func__, i);
                        goto out;
                }
                gpio_direction_output(GPIO5_SCL_NUM, 0);
                udelay(5);
        }
        pr_err("%s failed to recovery i2c\n", __func__);
out:
        gpio_direction_output(GPIO4_SDA_NUM, 1);
        gpio_direction_output(GPIO5_SCL_NUM, 1);
        udelay(1);
        gpio_free(GPIO4_SDA_NUM);
        gpio_free(GPIO5_SCL_NUM);
        p = devm_pinctrl_get_select(gi2c->dev, "default");
        if (IS_ERR(p)) {
                dev_err(gi2c->dev, "%s pinctrl_get fail ret = %d\n",
                                __func__, PTR_ERR(p));
			 ...
			 int num)
{
	...
	if ((geni_ios & 0x3) != 0x3) { //SCL:b'1, SDA:b'0
		...
		if (gi2c->se0_enable_recovery) {
			force_i2c0_scl(gi2c);
		}
		if (gi2c->se1_enable_recovery) {
			force_i2c1_scl(gi2c);
		}
		/* for levm skip auto suspend timer */
		if (!gi2c->is_le_vm) {
...
static int geni_i2c_probe(struct platform_device *pdev)
{
	...
	if (res->start == 0x4a80000) {
		gi2c->se0_enable_recovery = true;
		pr_err("enable_recovery is set at address 0x%x\n",
				res->start);
	}
	if (res->start == 0x4a84000) {
		gi2c->se1_enable_recovery = true;
		pr_err("enable_recovery is set at address 0x%x\n",
				res->start);
	}
